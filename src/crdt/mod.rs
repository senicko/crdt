...
/// GCounter CRDT Replica with client side interface.
...
impl GCounterReplica {
    /// Creates a new GCounter replica for the given node_id.
    ...
    pub fn inc(&mut self, delta: u64) {
        self.crdt.inc(&self.node_id, delta);
    }
...
mod tests {
    ...
    fn test_gcounter_replica_sync() {
        let g1_node = Uuid::new_v4().to_string();
        let mut g1 = GCounterReplica::new(g1_node);

        let g2_node = Uuid::new_v4().to_string();
        let mut g2 = GCounterReplica::new(g2_node);
